#!/usr/bin/env python3
"""
run_multinli_finetune_irm.py
============================
Fine-tune les dernières couches de BERT/DistilBERT avec IRMv1, en utilisant
le genre MNLI + SNLI comme environnements. Compare avec un fine-tuning ERM
classique.

Architecture :
    [BERT couches 0..N-k gelées] → [couches N-k..N entraînables] → CLS → tête linéaire → 3 classes

Le modèle complet (BERT + tête) est le classifieur final : pas de second
stage d'entraînement. On évalue directement l'accuracy OOD.

Environnements d'entraînement (6) :
    MNLI : fiction, government, slate, telephone, travel
    SNLI : snli

Évaluation OOD :
    - MNLI validation_matched   (ID)
    - MNLI validation_mismatched (OOD)
    - ANLI R1 / R2 / R3          (OOD)

Usage :
    uv run real/multinli/run_multinli_finetune_irm.py --device auto
    uv run real/multinli/run_multinli_finetune_irm.py --irm_lambda 100 --epochs 3
"""
from __future__ import annotations

import sys
from pathlib import Path as _Path

_ROOT = _Path(__file__).resolve().parents[2]
for _p in [str(_ROOT), str(_ROOT / "shared")]:
    if _p not in sys.path:
        sys.path.insert(0, _p)

import argparse
import json
import os
from typing import Dict, List, Tuple

import matplotlib.pyplot as plt
import numpy as np
import torch
from torch import nn
from torch.autograd import grad
from torch.utils.data import DataLoader, Dataset

from utils_irm import resolve_device


# =============================================================================
# 1. Chargement des datasets
# =============================================================================

TRAIN_GENRES = ["fiction", "government", "slate", "telephone", "travel"]
ANLI_ROUNDS = ["r1", "r2", "r3"]


def load_all_datasets() -> Tuple[dict, dict, dict]:
    """Charge MultiNLI, SNLI et ANLI depuis Hugging Face."""
    from datasets import load_dataset

    print("Chargement de MultiNLI …")
    mnli = load_dataset("nyu-mll/multi_nli")
    print(f"  train: {len(mnli['train']):,}  val_matched: {len(mnli['validation_matched']):,}  "
          f"val_mismatched: {len(mnli['validation_mismatched']):,}")

    print("Chargement de SNLI …")
    snli = load_dataset("stanfordnlp/snli")
    print(f"  train: {len(snli['train']):,}")

    print("Chargement de ANLI …")
    anli = load_dataset("facebook/anli")
    for r in ANLI_ROUNDS:
        print(f"  test_{r}: {len(anli[f'test_{r}']):,}")

    return mnli, snli, anli


# =============================================================================
# 2. Dataset & DataLoader par environnement
# =============================================================================

class _NLIPairDataset(Dataset):
    """Dataset NLI : tokenise les paires (premise, hypothesis) à la volée."""

    def __init__(self, premises, hypotheses, labels, tokenizer, max_length):
        self.premises   = premises
        self.hypotheses = hypotheses
        self.labels     = labels
        self.tokenizer  = tokenizer
        self.max_length = max_length

    def __len__(self):
        return len(self.labels)

    def __getitem__(self, idx):
        enc = self.tokenizer(
            self.premises[idx],
            self.hypotheses[idx],
            max_length=self.max_length,
            truncation=True,
            padding=False,
        )
        item = {
            "input_ids":      enc["input_ids"],
            "attention_mask": enc["attention_mask"],
            "label":          self.labels[idx],
        }
        if "token_type_ids" in enc:
            item["token_type_ids"] = enc["token_type_ids"]
        return item


def _extract_env_data(split_ds, genre: str | None = None):
    """Extrait (premises, hypotheses, labels) depuis un split HF, filtre par genre si fourni."""
    labels_col   = split_ds["label"]
    premises_col = split_ds["premise"]
    hyp_col      = split_ds["hypothesis"]
    genres_col   = split_ds["genre"] if "genre" in split_ds.column_names else None

    premises, hypotheses, labels = [], [], []
    for idx, label in enumerate(labels_col):
        if label == -1:
            continue
        if genre is not None and genres_col is not None and genres_col[idx] != genre:
            continue
        premises.append(premises_col[idx])
        hypotheses.append(hyp_col[idx])
        labels.append(label)
    return premises, hypotheses, labels


def build_env_loaders(
    mnli, snli, tokenizer, max_length: int, batch_size: int,
    snli_max_samples: int = 90_000,
    seed: int = 42,
) -> Dict[str, DataLoader]:
    """Construit un DataLoader par environnement d'entraînement.

    SNLI est sous-échantillonné à snli_max_samples pour équilibrer les envs
    avec les genres MNLI (~80k exemples chacun).
    """
    rng = np.random.default_rng(seed)
    envs = {}

    # 5 genres MNLI
    for genre in TRAIN_GENRES:
        p, h, l = _extract_env_data(mnli["train"], genre=genre)
        ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
        envs[genre] = DataLoader(ds, batch_size=batch_size, shuffle=True,
                                 num_workers=0, collate_fn=_make_collate(tokenizer),
                                 pin_memory=True, drop_last=False)
        print(f"  Env {genre:12s} : {len(ds):,} paires")

    # SNLI — sous-échantillonné
    p, h, l = _extract_env_data(snli["train"])
    if snli_max_samples and len(l) > snli_max_samples:
        idx = rng.choice(len(l), size=snli_max_samples, replace=False)
        p = [p[i] for i in idx]
        h = [h[i] for i in idx]
        l = [l[i] for i in idx]
    ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
    envs["snli"] = DataLoader(ds, batch_size=batch_size, shuffle=True,
                              num_workers=0, collate_fn=_make_collate(tokenizer),
                              pin_memory=True, drop_last=False)
    print(f"  Env {'snli':12s} : {len(ds):,} paires (max={snli_max_samples:,})")

    return envs


def build_eval_loaders(
    mnli, anli, tokenizer, max_length: int, batch_size: int,
) -> Dict[str, DataLoader]:
    """Construit les DataLoaders d'évaluation."""
    evals = {}
    collate = _make_collate(tokenizer)

    # Val matched (ID) — global
    p, h, l = _extract_env_data(mnli["validation_matched"])
    ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
    evals["val_matched"] = DataLoader(ds, batch_size=batch_size, shuffle=False,
                                      num_workers=0, collate_fn=collate)

    # Val matched par genre (pour le plot détaillé)
    for genre in TRAIN_GENRES:
        p, h, l = _extract_env_data(mnli["validation_matched"], genre=genre)
        ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
        evals[f"val_{genre}"] = DataLoader(ds, batch_size=batch_size, shuffle=False,
                                           num_workers=0, collate_fn=collate)

    # Val mismatched (OOD)
    p, h, l = _extract_env_data(mnli["validation_mismatched"])
    ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
    evals["val_mismatched"] = DataLoader(ds, batch_size=batch_size, shuffle=False,
                                         num_workers=0, collate_fn=collate)

    # ANLI rounds
    for r in ANLI_ROUNDS:
        p, h, l = _extract_env_data(anli[f"test_{r}"])
        ds = _NLIPairDataset(p, h, l, tokenizer, max_length)
        evals[f"anli_{r}"] = DataLoader(ds, batch_size=batch_size, shuffle=False,
                                        num_workers=0, collate_fn=collate)

    return evals


def _make_collate(tokenizer):
    """Crée une fonction collate qui utilise le padding dynamique du tokenizer."""
    def _collate(batch):
        labels_b = [item.pop("label") for item in batch]
        padded = tokenizer.pad(batch, return_tensors="pt")
        padded["labels"] = torch.tensor(labels_b, dtype=torch.long)
        return padded
    return _collate


# =============================================================================
# 3. Modèle BERT + tête
# =============================================================================

class BertNLIModel(nn.Module):
    """BERT/DistilBERT + dropout + tête linéaire pour NLI 3-classes."""

    def __init__(self, backbone: nn.Module, hidden_size: int, n_classes: int = 3):
        super().__init__()
        self.backbone   = backbone
        self.dropout    = nn.Dropout(0.1)
        self.classifier = nn.Linear(hidden_size, n_classes)

    def forward(self, input_ids, attention_mask, token_type_ids=None):
        kw = dict(input_ids=input_ids, attention_mask=attention_mask)
        if token_type_ids is not None:
            kw["token_type_ids"] = token_type_ids
        out = self.backbone(**kw)
        cls = out.last_hidden_state[:, 0, :]
        return self.classifier(self.dropout(cls))


def _get_layer_modules(backbone) -> list:
    """Retourne la liste ordonnée des couches Transformer du backbone."""
    # BERT : backbone.encoder.layer[i]
    if hasattr(backbone, "encoder") and hasattr(backbone.encoder, "layer"):
        return list(backbone.encoder.layer)
    # DistilBERT : backbone.transformer.layer[i]
    if hasattr(backbone, "transformer") and hasattr(backbone.transformer, "layer"):
        return list(backbone.transformer.layer)
    raise ValueError("Architecture non reconnue : impossible d'identifier les couches Transformer.")


def freeze_backbone_except_last_n(backbone: nn.Module, n_unfrozen: int = 2):
    """Gèle tout le backbone sauf les n dernières couches Transformer."""
    # Tout geler
    for p in backbone.parameters():
        p.requires_grad = False

    # Dégeler les n dernières couches
    layers = _get_layer_modules(backbone)
    n_layers = len(layers)
    for i in range(max(0, n_layers - n_unfrozen), n_layers):
        for p in layers[i].parameters():
            p.requires_grad = True

    n_total = sum(p.numel() for p in backbone.parameters())
    n_train = sum(p.numel() for p in backbone.parameters() if p.requires_grad)
    print(f"  Backbone : {n_layers} couches, {n_unfrozen} dégelées")
    print(f"  Params   : {n_train:,} / {n_total:,} entraînables ({100*n_train/n_total:.1f}%)")


# =============================================================================
# 4. Évaluation
# =============================================================================

@torch.no_grad()
def evaluate_loader(model: nn.Module, loader: DataLoader, device: torch.device) -> float:
    """Accuracy sur un DataLoader."""
    model.eval()
    correct, total = 0, 0
    for batch in loader:
        input_ids      = batch["input_ids"].to(device)
        attention_mask = batch["attention_mask"].to(device)
        token_type_ids = batch.get("token_type_ids")
        if token_type_ids is not None:
            token_type_ids = token_type_ids.to(device)
        labels_b = batch["labels"].to(device)

        logits = model(input_ids, attention_mask, token_type_ids)
        correct += (logits.argmax(-1) == labels_b).sum().item()
        total   += len(labels_b)
    return correct / total if total > 0 else 0.0


def evaluate_all(
    model: nn.Module, eval_loaders: Dict[str, DataLoader], device: torch.device,
) -> dict:
    """Évalue le modèle sur tous les loaders d'évaluation."""
    results = {}
    for name, loader in eval_loaders.items():
        results[name] = evaluate_loader(model, loader, device)
    return results


# =============================================================================
# 5. Fine-tuning ERM (baseline)
# =============================================================================

def finetune_erm(
    model: BertNLIModel,
    env_loaders: Dict[str, DataLoader],
    eval_loaders: Dict[str, DataLoader],
    epochs: int,
    lr_bert: float,
    lr_head: float,
    device: torch.device,
    eval_every_steps: int = 500,
) -> Tuple[dict, list]:
    """
    Fine-tune ERM : cross-entropie standard sur les données poolées de tous les envs.
    """
    from transformers import get_linear_schedule_with_warmup

    # Deux groupes de paramètres avec LR différents
    bert_params = [p for p in model.backbone.parameters() if p.requires_grad]
    head_params = list(model.classifier.parameters())
    optimizer = torch.optim.AdamW([
        {"params": bert_params, "lr": lr_bert, "weight_decay": 0.01},
        {"params": head_params, "lr": lr_head, "weight_decay": 0.01},
    ])

    # 1 step = 1 batch par env. On itère sur le plus grand env
    # pour que chaque env voie toutes ses données au moins 1× par epoch.
    # Les petits envs recyclent automatiquement via StopIteration.
    steps_per_epoch = max(len(loader) for loader in env_loaders.values())
    total_steps = epochs * steps_per_epoch
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=int(0.06 * total_steps),
        num_training_steps=total_steps,
    )

    loss_fn = nn.CrossEntropyLoss()
    use_autocast = "cuda" in str(device)
    history = []

    print(f"\n  Fine-tuning ERM — {epochs} epochs, {total_steps:,} steps total")
    print(f"  lr_bert={lr_bert}, lr_head={lr_head}")

    global_step = 0
    for epoch in range(epochs):
        model.train()
        total_loss, correct, total = 0.0, 0, 0

        # Créer des itérateurs infinis pour chaque env
        env_iters = {name: iter(loader) for name, loader in env_loaders.items()}
        env_names = list(env_loaders.keys())

        # Itérer sur le nombre total de batches de l'epoch
        for _ in range(steps_per_epoch):
            # Tirer un batch de chaque env et accumuler la loss
            batch_loss = torch.tensor(0.0, device=device)
            n_samples = 0

            for name in env_names:
                try:
                    batch = next(env_iters[name])
                except StopIteration:
                    env_iters[name] = iter(env_loaders[name])
                    batch = next(env_iters[name])

                input_ids      = batch["input_ids"].to(device)
                attention_mask = batch["attention_mask"].to(device)
                token_type_ids = batch.get("token_type_ids")
                if token_type_ids is not None:
                    token_type_ids = token_type_ids.to(device)
                labels_b = batch["labels"].to(device)

                with torch.amp.autocast("cuda", enabled=use_autocast):
                    logits = model(input_ids, attention_mask, token_type_ids)
                    loss_e = loss_fn(logits, labels_b)
                batch_loss = batch_loss + loss_e
                correct += (logits.detach().argmax(-1) == labels_b).sum().item()
                n_samples += len(labels_b)

            # Moyenne sur les envs
            batch_loss = batch_loss / len(env_names)

            optimizer.zero_grad()
            batch_loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            scheduler.step()

            total_loss += batch_loss.item()
            total      += n_samples
            global_step += 1

            if eval_every_steps and global_step % eval_every_steps == 0:
                res = evaluate_all(model, eval_loaders, device)
                history.append({"step": global_step, "epoch": epoch + 1, **res})
                vm = res.get("val_matched", 0)
                vmm = res.get("val_mismatched", 0)
                ar1 = res.get("anli_r1", 0)
                print(f"    [ERM] step {global_step:,}  loss={total_loss/max(1,global_step - epoch*steps_per_epoch):.4f}  "
                      f"val_m={vm:.4f}  val_mm={vmm:.4f}  anli_r1={ar1:.4f}")
                model.train()

        epoch_loss = total_loss / steps_per_epoch
        epoch_acc  = correct / total if total > 0 else 0
        print(f"  → Epoch {epoch+1}/{epochs}  loss={epoch_loss:.4f}  train_acc={epoch_acc:.4f}")

    final_results = evaluate_all(model, eval_loaders, device)
    return final_results, history


# =============================================================================
# 6. Fine-tuning IRM (IRMv1)
# =============================================================================

def finetune_irm(
    model: BertNLIModel,
    env_loaders: Dict[str, DataLoader],
    eval_loaders: Dict[str, DataLoader],
    epochs: int,
    lr_bert: float,
    lr_head: float,
    irm_lambda: float,
    warmup_fraction: float,
    device: torch.device,
    eval_every_steps: int = 500,
) -> Tuple[dict, list]:
    """
    Fine-tune IRMv1 : à chaque step, 1 mini-batch par env, loss ERM + λ·pénalité.

    La pénalité IRMv1 pour un env e est : ‖∇_w [w · CE(w·logits_e, y_e)]‖²
    avec w = scalaire dummy initialisé à 1.
    """
    from transformers import get_linear_schedule_with_warmup

    # Deux groupes de paramètres avec LR différents
    bert_params = [p for p in model.backbone.parameters() if p.requires_grad]
    head_params = list(model.classifier.parameters())
    optimizer = torch.optim.AdamW([
        {"params": bert_params, "lr": lr_bert, "weight_decay": 0.01},
        {"params": head_params, "lr": lr_head, "weight_decay": 0.01},
    ])

    # Le step = 1 forward par env. On aligne sur le plus grand env pour
    # que chaque env voie toutes ses données au moins 1× par epoch.
    max_loader_len = max(len(loader) for loader in env_loaders.values())
    steps_per_epoch = max_loader_len
    total_steps = epochs * steps_per_epoch
    warmup_steps = int(warmup_fraction * total_steps)

    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=int(0.06 * total_steps),
        num_training_steps=total_steps,
    )

    loss_fn = nn.CrossEntropyLoss()
    use_autocast = "cuda" in str(device)
    history = []
    E = len(env_loaders)

    print(f"\n  Fine-tuning IRM — {epochs} epochs, {total_steps:,} steps total")
    print(f"  lr_bert={lr_bert}, lr_head={lr_head}, λ_max={irm_lambda}")
    print(f"  warmup: {warmup_steps:,} steps ({warmup_fraction*100:.0f}%)")
    print(f"  steps/epoch: {steps_per_epoch:,} (limité par le plus petit env)")

    global_step = 0
    for epoch in range(epochs):
        model.train()
        total_loss, total_penalty = 0.0, 0.0
        correct, total = 0, 0

        env_iters = {name: iter(loader) for name, loader in env_loaders.items()}
        env_names = list(env_loaders.keys())

        for _ in range(steps_per_epoch):
            emp_risk = torch.tensor(0.0, device=device)
            penalties = []

            for name in env_names:
                try:
                    batch = next(env_iters[name])
                except StopIteration:
                    env_iters[name] = iter(env_loaders[name])
                    batch = next(env_iters[name])

                input_ids      = batch["input_ids"].to(device)
                attention_mask = batch["attention_mask"].to(device)
                token_type_ids = batch.get("token_type_ids")
                if token_type_ids is not None:
                    token_type_ids = token_type_ids.to(device)
                labels_b = batch["labels"].to(device)

                with torch.amp.autocast("cuda", enabled=use_autocast):
                    logits = model(input_ids, attention_mask, token_type_ids)
                    loss_e = loss_fn(logits, labels_b)

                emp_risk = emp_risk + loss_e

                # Pénalité IRMv1 : ∇_w [w · loss_e(w·logits)]²
                scale = torch.tensor(1.0, device=device, requires_grad=True)
                loss_scaled = loss_fn(logits * scale, labels_b)
                grad_scale = grad(loss_scaled, [scale], create_graph=True)[0]
                penalties.append(grad_scale ** 2)

                correct += (logits.detach().argmax(-1) == labels_b).sum().item()
                total   += len(labels_b)

            emp_risk = emp_risk / E
            penalty  = torch.stack(penalties).mean()

            # Warmup linéaire de λ
            if global_step < warmup_steps:
                lambda_t = irm_lambda * (global_step / warmup_steps)
            else:
                lambda_t = irm_lambda

            objective = emp_risk + lambda_t * penalty

            # Normalisation pour stabiliser les gradients quand λ est grand
            if lambda_t > 1.0:
                objective = objective / lambda_t

            optimizer.zero_grad()
            objective.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            scheduler.step()

            total_loss    += emp_risk.item()
            total_penalty += penalty.item()
            global_step   += 1

            if eval_every_steps and global_step % eval_every_steps == 0:
                res = evaluate_all(model, eval_loaders, device)
                history.append({"step": global_step, "epoch": epoch + 1,
                                "penalty": total_penalty / (global_step - epoch * steps_per_epoch),
                                **res})
                vm = res.get("val_matched", 0)
                vmm = res.get("val_mismatched", 0)
                ar1 = res.get("anli_r1", 0)
                print(f"    [IRM] step {global_step:,}  loss={total_loss/max(1,global_step - epoch*steps_per_epoch):.4f}  "
                      f"pen={total_penalty/max(1,global_step - epoch*steps_per_epoch):.4f}  "
                      f"λ={lambda_t:.1f}  val_m={vm:.4f}  val_mm={vmm:.4f}  anli_r1={ar1:.4f}")
                model.train()

        epoch_loss = total_loss / steps_per_epoch
        epoch_acc  = correct / total if total > 0 else 0
        print(f"  → Epoch {epoch+1}/{epochs}  loss={epoch_loss:.4f}  train_acc={epoch_acc:.4f}")

    final_results = evaluate_all(model, eval_loaders, device)
    return final_results, history


# =============================================================================
# 7. Visualisation
# =============================================================================

def plot_comparison(results_erm: dict, results_irm: dict, out_dir: str):
    """Bar chart comparant ERM FT vs IRM FT sur tous les sets d'évaluation."""
    sets = ["val_matched", "val_mismatched", "anli_r1", "anli_r2", "anli_r3"]
    labels = ["Val Matched\n(ID)", "Val Mismatched\n(OOD)", "ANLI R1", "ANLI R2", "ANLI R3"]
    x = np.arange(len(sets))
    width = 0.3

    erm_accs = [results_erm.get(s, 0) for s in sets]
    irm_accs = [results_irm.get(s, 0) for s in sets]

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.bar(x - width / 2, erm_accs, width, label="ERM fine-tune", color="#e74c3c", edgecolor="black")
    ax.bar(x + width / 2, irm_accs, width, label="IRM fine-tune", color="#2ecc71", edgecolor="black")

    for i, (e, r) in enumerate(zip(erm_accs, irm_accs)):
        ax.text(i - width / 2, e + 0.005, f"{e:.3f}", ha="center", fontsize=9)
        ax.text(i + width / 2, r + 0.005, f"{r:.3f}", ha="center", fontsize=9)

    ax.axvline(x=0.5, color="gray", ls="--", alpha=0.5)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Accuracy")
    ax.set_title("BERT fine-tuné ERM vs IRM — accuracy OOD")
    ax.legend()
    ax.set_ylim(0, 1.05)
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "finetune_comparison.png"), dpi=150)
    plt.close()
    print(f"  Plot sauvegardé dans {out_dir}/finetune_comparison.png")


def plot_per_genre(results_erm: dict, results_irm: dict, out_dir: str):
    """Accuracy par genre (val matched) + OOD."""
    genre_keys = [f"val_{g}" for g in TRAIN_GENRES]
    ood_keys   = ["val_mismatched", "anli_r1", "anli_r2", "anli_r3"]
    all_keys   = genre_keys + ood_keys
    all_labels = TRAIN_GENRES + ["Mismatched", "ANLI R1", "ANLI R2", "ANLI R3"]

    erm_accs = [results_erm.get(k, 0) for k in all_keys]
    irm_accs = [results_irm.get(k, 0) for k in all_keys]

    x = np.arange(len(all_labels))
    width = 0.35

    fig, ax = plt.subplots(figsize=(14, 5))
    bars_erm = ax.bar(x - width / 2, erm_accs, width, label="ERM fine-tune",
                      color="#e74c3c", edgecolor="black", alpha=0.85)
    bars_irm = ax.bar(x + width / 2, irm_accs, width, label="IRM fine-tune",
                      color="#2ecc71", edgecolor="black", alpha=0.85)

    for bar, v in zip(bars_erm, erm_accs):
        ax.text(bar.get_x() + bar.get_width() / 2, v + 0.005, f"{v:.3f}",
                ha="center", fontsize=8)
    for bar, v in zip(bars_irm, irm_accs):
        ax.text(bar.get_x() + bar.get_width() / 2, v + 0.005, f"{v:.3f}",
                ha="center", fontsize=8)

    ax.axvline(x=len(TRAIN_GENRES) - 0.5, color="gray", ls="--", alpha=0.5)
    ax.text(len(TRAIN_GENRES) - 0.75, 0.98, "ID ←", ha="right", fontsize=9,
            color="gray", transform=ax.get_xaxis_transform())
    ax.text(len(TRAIN_GENRES) - 0.25, 0.98, "→ OOD", ha="left", fontsize=9,
            color="gray", transform=ax.get_xaxis_transform())

    ax.set_xticks(x)
    ax.set_xticklabels(all_labels, rotation=15, ha="right")
    ax.set_ylabel("Accuracy")
    ax.set_title("BERT fine-tuné — accuracy par genre / round (ERM vs IRM)")
    ax.legend()
    ax.set_ylim(0, 1.05)
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "finetune_per_genre.png"), dpi=150)
    plt.close()
    print(f"  Per-genre plot sauvegardé dans {out_dir}/finetune_per_genre.png")


def plot_training_history(hist_erm: list, hist_irm: list, out_dir: str):
    """Courbes d'accuracy pendant le fine-tuning (val_matched, val_mismatched, ANLI R1)."""
    if not hist_erm or not hist_irm:
        return

    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    metrics = [
        ("val_matched", "Val Matched (ID)"),
        ("val_mismatched", "Val Mismatched (OOD)"),
        ("anli_r1", "ANLI R1 (OOD)"),
    ]

    for ax, (key, title) in zip(axes, metrics):
        steps_erm = [h["step"] for h in hist_erm if key in h]
        vals_erm  = [h[key] for h in hist_erm if key in h]
        steps_irm = [h["step"] for h in hist_irm if key in h]
        vals_irm  = [h[key] for h in hist_irm if key in h]

        ax.plot(steps_erm, vals_erm, label="ERM FT", alpha=0.8, color="C0")
        ax.plot(steps_irm, vals_irm, label="IRM FT", alpha=0.8, color="C1")
        ax.set_xlabel("Step")
        ax.set_ylabel("Accuracy")
        ax.set_title(title)
        ax.legend()
        ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "finetune_curves.png"), dpi=150)
    plt.close()
    print(f"  Courbes sauvegardées dans {out_dir}/finetune_curves.png")


# =============================================================================
# MAIN
# =============================================================================

def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune BERT avec IRMv1 sur MultiNLI+SNLI (genre = env)"
    )
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--device", type=str, default="auto")
    parser.add_argument("--bert_model", type=str, default="bert-base-uncased")
    parser.add_argument("--max_length", type=int, default=256)

    # Fine-tuning
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--batch_size", type=int, default=16,
                        help="Batch size par env. Total = batch_size × 6 envs par step.")
    parser.add_argument("--lr_bert", type=float, default=2e-5,
                        help="LR pour les couches BERT dégelées.")
    parser.add_argument("--lr_head", type=float, default=1e-3,
                        help="LR pour la tête de classification.")
    parser.add_argument("--n_unfrozen_layers", type=int, default=2,
                        help="Nombre de couches Transformer à dégeler (depuis la fin).")

    # IRM
    parser.add_argument("--irm_lambda", type=float, default=100.0,
                        help="Poids maximal de la pénalité IRM.")
    parser.add_argument("--warmup_fraction", type=float, default=0.1,
                        help="Fraction des steps en warmup linéaire de λ.")

    # Évaluation
    parser.add_argument("--eval_every", type=int, default=500,
                        help="Évaluer tous les N steps pendant le fine-tuning.")

    # Output
    parser.add_argument("--out_dir", type=str,
                        default=str(_Path(__file__).parent / "plots_finetune_irm"))
    parser.add_argument("--snli_max_samples", type=int, default=90_000,
                        help="Sous-échantillonnage de SNLI pour équilibrer les envs. "
                             "0 = pas de limite.")

    args = parser.parse_args()
    device = torch.device(resolve_device(args.device))
    os.makedirs(args.out_dir, exist_ok=True)
    torch.manual_seed(args.seed)
    n_classes = 3

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 1 : Chargement des datasets")
    print("=" * 70)
    mnli, snli, anli = load_all_datasets()

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 2 : Construction des DataLoaders")
    print("=" * 70)
    from transformers import AutoTokenizer, AutoModel

    tokenizer = AutoTokenizer.from_pretrained(args.bert_model)

    print("\nEnvironnements d'entraînement :")
    env_loaders = build_env_loaders(
        mnli, snli, tokenizer, args.max_length, args.batch_size,
        snli_max_samples=args.snli_max_samples,
        seed=args.seed,
    )

    print("\nSets d'évaluation :")
    eval_loaders = build_eval_loaders(mnli, anli, tokenizer, args.max_length,
                                      batch_size=args.batch_size * 4)
    for name, loader in eval_loaders.items():
        print(f"  {name:20s} : {len(loader.dataset):,} paires")

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 3 : Fine-tuning ERM (baseline)")
    print("=" * 70)

    backbone_erm = AutoModel.from_pretrained(args.bert_model)
    model_erm = BertNLIModel(backbone_erm, backbone_erm.config.hidden_size, n_classes).to(device)
    freeze_backbone_except_last_n(model_erm.backbone, args.n_unfrozen_layers)

    results_erm, hist_erm = finetune_erm(
        model_erm, env_loaders, eval_loaders,
        epochs=args.epochs,
        lr_bert=args.lr_bert,
        lr_head=args.lr_head,
        device=device,
        eval_every_steps=args.eval_every,
    )

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 4 : Fine-tuning IRM")
    print("=" * 70)

    # Nouveau modèle initialisé depuis les poids pré-entraînés (pas depuis ERM)
    torch.manual_seed(args.seed)
    backbone_irm = AutoModel.from_pretrained(args.bert_model)
    model_irm = BertNLIModel(backbone_irm, backbone_irm.config.hidden_size, n_classes).to(device)
    freeze_backbone_except_last_n(model_irm.backbone, args.n_unfrozen_layers)

    results_irm, hist_irm = finetune_irm(
        model_irm, env_loaders, eval_loaders,
        epochs=args.epochs,
        lr_bert=args.lr_bert,
        lr_head=args.lr_head,
        irm_lambda=args.irm_lambda,
        warmup_fraction=args.warmup_fraction,
        device=device,
        eval_every_steps=args.eval_every,
    )

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 5 : Résultats")
    print("=" * 70)

    header_keys = ["val_matched", "val_mismatched", "anli_r1", "anli_r2", "anli_r3"]
    header_labels = ["Val ID", "Mismatch", "ANLI R1", "ANLI R2", "ANLI R3"]
    print(f"\n  {'':12s}  " + "  ".join(f"{h:>8s}" for h in header_labels))
    print(f"  {'':12s}  " + "  ".join("─" * 8 for _ in header_labels))

    for name, res in [("ERM FT", results_erm), ("IRM FT", results_irm)]:
        vals = [f"{res.get(k, 0):8.4f}" for k in header_keys]
        print(f"  {name:12s}  " + "  ".join(vals))

    # Per-genre
    print("\n  Détail par genre (val_matched) :")
    for g in TRAIN_GENRES:
        k = f"val_{g}"
        acc_erm = results_erm.get(k, 0)
        acc_irm = results_irm.get(k, 0)
        print(f"    {g:12s}  ERM={acc_erm:.4f}  IRM={acc_irm:.4f}")

    # Sauvegarder
    all_results = {"erm_ft": results_erm, "irm_ft": results_irm}
    with open(os.path.join(args.out_dir, "results.json"), "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\n  Résultats sauvegardés dans {args.out_dir}/results.json")

    # ─────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ÉTAPE 6 : Plots")
    print("=" * 70)

    plot_comparison(results_erm, results_irm, args.out_dir)
    plot_per_genre(results_erm, results_irm, args.out_dir)
    plot_training_history(hist_erm, hist_irm, args.out_dir)

    print("\n✓ Terminé.")


if __name__ == "__main__":
    main()
