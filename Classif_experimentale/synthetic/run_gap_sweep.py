...
Paramètre variant par dataset :
  ...
  synthetic_confounding_varying_pc    : p_c      (center=0.9,  test=0.1)
...
Usage :
    ...
    uv run run_gap_sweep.py --dataset synthetic_confounding_varying_pc --irm_lambda 500
"""
...
def _build(gap):
    ...
    if args.dataset == 'synthetic_semi_anti_causal':
        ...
        envs = build_envs_confounding_varying_pc(
            n=args.n, pc_train=[p1, p2], pc_test=g_test,
            a=args.conf_pc_a, gamma=args.conf_pc_gamma,
            seed=args.seed, val_frac=args.val_frac, n_test=args.n_test,
            dim_z=args.dim_z, dim_y=args.dim_y,
        )
    # ── Anti-causal variants ──
        ...
        envs = build_envs_anti_causal_confounding_varying_gamma(
            n=args.n, train_gammas=[p1, p2], test_gamma=g_test,
            seed=args.seed, val_frac=args.val_frac, n_test=args.n_test,
            dim_z=args.dim_z, dim_y=args.dim_y,
        )
    elif args.dataset == 'synthetic_ac_confounding_varying_pc':
        envs = build_envs_anti_causal_confounding_varying_pc(
            n=args.n, pc_train=[p1, p2], pc_test=g_test,
            a=args.conf_pc_a, gamma=args.conf_pc_gamma,
            seed=args.seed, val_frac=args.val_frac, n_test=args.n_test,
            dim_z=args.dim_z, dim_y=args.dim_y,
