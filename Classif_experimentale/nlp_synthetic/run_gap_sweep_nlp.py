...
====================
Balayage du GAP entre les deux environnements de train pour les datasets NLP.

Le "gap" est la différence entre le paramètre d'environnement des deux envs
de train.  À gap=0 les deux envs sont identiques ; à gap élevé IRM exploite
l'incohérence inter-environnements.
...
def make_gap_sweep_nlp_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Gap sweep pour datasets NLP (AG News, IMDB Genres, Amazon)"
    )
    ...
    p.add_argument('--seeds',      type=int,   nargs='+', default=None,
                   help='Liste de seeds à exécuter (ex: 0 1 2). Surcharge --seed.')
...
def _build_envs(args, p1: float, p2: float):
    """Construit (train_envs, val_envs, test_env) pour le dataset et le gap donnés."""
    ...
    if args.dataset == 'nlp_agnews_semi_anti_causal':
        ...
        return build_envs_ag_news_source_selection(
        ...
        return build_envs_imdb_semi_anti_causal(
            train_p_correct=[p1, p2],
            test_p_correct=g_test,
            seed=args.seed,
            label_flip=flip,
        ...
        return build_envs_imdb_size_selection(
            train_p_select=[p1, p2],
            seed=args.seed,
            threshold_method=args.nlp_size_threshold_method,
            label_flip=flip,
        ...
        return build_envs_imdb_br_selection(
            train_p_select=[p1, p2],
            seed=args.seed,
            label_flip=flip,
            bert_model=bert, max_length=mxlen, device=dev, pooling=pool,
        ...
        return build_envs_imdb_conf_varying_proxy(
            a_train=[p1, p2],
            a_test=g_test,
            seed=args.seed,
            p_c_flip=args.nlp_conf_p_c_flip,
        ...
        return build_envs_amazon_size_selection(
            train_p_select=[p1, p2],
            seed=args.seed,
            label_flip=flip,
            n_target=args.nlp_amazon_n_target,
    ...
    raise ValueError(f"Dataset non supporté : {args.dataset}")
...
if __name__ == '__main__':
    ...
    # Appliquer les defaults du dataset si non fournis
    ...
    if args.out_dir is None:
        ...
        _SLUG_MAP = {
            ...
            'nlp_imdb_semi_anti_causal':           'causal_imdb_sac',
            'nlp_imdb_size_selection':             'causal_imdb_size_selection',
            'nlp_imdb_br_selection':               'causal_imdb_br_selection',
            'nlp_imdb_conf_varying_proxy':         'causal_imdb_conf_proxy',
    ...
    print(f"  paramètre : {defs['param_label']}")
    ...
    # results = liste de dicts avec mean ± std par gap
    results = []
    for i, gap in enumerate(gaps):
        ...
        results.append({
            ...
            'erm_test_final':  float(np.mean(erm_vals)),   # compat legacy
            'irm_test_final':  float(np.mean(irm_vals)),   # compat legacy
        })
    ...
    plt.close(fig)
    print(f"Plot sauvegardé : {fig_path}")
